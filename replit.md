...
- **April 2026 (Deep Cleanup)**: Deleted 28 unused shadcn UI components (alert-dialog, avatar, carousel, checkbox, dialog, dropdown-menu, form, select, sidebar, tabs, textarea, etc.). Removed 37 unused npm packages: drizzle-orm, drizzle-zod, pg, passport, express-session, ws, date-fns, embla-carousel, recharts, 20 Radix UI packages, etc. Deleted `drizzle.config.ts` (no database), `screenshots/` folder, old task files. Consolidated `SITE_URL` constant — single source of truth in `shared/seo-meta.ts`. Simplified `api/sitemap.ts` to reuse `shared/sitemap.ts`. Dependencies reduced from ~80 to 31 total.
